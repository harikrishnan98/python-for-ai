...
def download_single_image(url: str, img_num: int) -> Path: # Removed session params
    ...
    with download_path.open("wb") as f:
        # download in chunks (memory efficient)
...
async def download_images(urls: list) -> list[Path]: # Converting this as a async function to create a Task group for a sync fn
...
async def process_images(orig_paths: list[Path]) -> list[Path]:
    ...
    async with asyncio.TaskGroup() as tg:
        tasks_process = [tg.create_task(asyncio.to_thread(process_single_image, orig_path)) for orig_path in orig_paths]

    img_paths = [task.result() for task in tasks_process]
